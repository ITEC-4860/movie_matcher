...
class InfiniteScrolling extends React.Component {
    ...
    render() {
        return (
            ...
            >
                <div className={"InfScrollMain"}>
                    {this.state.items.map((i) => {
                        return (
                            <div key={i.id} className={"InfScrollCard"}>
                                <img key={i['poster_path']} alt={`A poster for ${i.title}`}
                                     src={`https://image.tmdb.org/t/p/w500${i['backdrop_path']}`}/>
                                <div className={"Title"}>{i.title}</div>
                                <div className={"CardInfo"}>Rating: {i['vote_average']}</div>
                                <div className={"CardInfo"}>Released: {this.formatDate(i['release_date'])}</div>
                                <Popup contentStyle={
                                    {width: "40%", backgroundColor: "grey"}
                                } trigger={<button> See More </button>} position={"right top"}>
                                    {void console.log(i.id)}
